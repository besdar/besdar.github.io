...
export function debounce(func: () => void, wait = 1000) {
...
type ReducedMotionMedia = Pick<MediaQueryList, "addEventListener" | "matches">;

export function initSnow(reducedMotionMedia: ReducedMotionMedia = window.matchMedia("(prefers-reduced-motion: reduce)")) {
    const canvas = document.createElement("canvas");
    canvas.role = "presentation";
    canvas.className = styles["snow-canvas"];
